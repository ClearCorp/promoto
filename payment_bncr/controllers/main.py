...
class BNCRController(http.Controller):
    ...
    def bncr_return_from_checkout(self, **data):
        ...
        _logger.info("Handling redirection from BNCR with data:\n%s", pprint.pformat(data))
    ...
    def bncr_webhook(self, reference, **data):
        """ Process the notification data sent by BNCR to the webhook.
        ...
        _logger.info("Webhook received from BNCR with data:\n%s", pprint.pformat(data))
        ...
        try:
            ...
            tx_sudo._handle_notification_data('bncr', data)

        except ValidationError:  # Acknowledge the notification to avoid getting spammed.
            _logger.exception("Unable to handle the BNCR  notification data; skipping to acknowledge")
    ...
    def _verify_notification_signature(notification_data, tx_sudo):
        ...
        if not received_signature:
            _logger.warning("BNCR: Received notification with missing signature.")
            raise Forbidden()

        # Compare the received signature with the expected signature computed from the data.
        expected_signature = tx_sudo.provider_id._bncr_calculate_signature(
            notification_data
        )
        if not hmac.compare_digest(received_signature, expected_signature):
            _logger.warning("BNCR:Received notification with invalid signature.")
            raise Forbidden()
